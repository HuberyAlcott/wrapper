...
/** @author Marcus */
@Slf4j
public class ImageTools extends FilesTools {
   ...
   * @return 图片缓存对象
   */
  public static BufferedImage verifyCode(
      int type,
      int length,
      String exclude,
      int width,
      ...
      Color foreColor,
      Color lineColor) {
    final var textCode = generateTextCode(type, length, exclude);
   ...
   * @return 验证码字符串
   */
  private static String generateTextCode(int type, int length, String excludeString) {
    if (length <= 0) {
      return "";
    }
    final var verifyCode = new StringBuilder();
    int i = 0;
    final var random = ThreadLocalRandom.current();
    switch (type) {
      case TYPE_NUM_ONLY:
  ...
  public static BufferedImage verifyCode(final Integer length) {
    final var textCode = verifyText(length);
    final var width = 120;
    final var height = 50;
    final var interLine = 7;
    ...
    /*写验证码字符*/
    for (int i = 0; i < textCode.length(); i++) {
      int fy = (int) ((Math.random() * 0.3 + 0.6) * height);
      graphics.setColor(generateRandomColor());
      /*将验证码字符显示到图象中*/
      graphics.drawString(textCode.charAt(i) + "", fx, fy);
      fx += fsize * 0.9;
  ...
  public static String verifyText(final Integer length) {
